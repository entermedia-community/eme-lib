...
public class FileManagerModule extends BaseModule
{
	...
	/**
	 * Construct one parameter dynamically from the values of other parameters. Example:
	 * <input type="hidden" name="param-part" value= "${page.filerootpath}/index.html?path=" />
	 * <input type="hidden" name="param-part" value="req.path" /> <input name="path"/>
	...
	public void movePage(WebPageRequest inReq) throws OpenEditException
	{
		...
		{
			throw new PageAlreadyExistsException("The destination path is the same as the source path");
		}
	...
	public void jumpToPage(WebPageRequest inReq) throws OpenEditException
	{
		...
		/*
		 * Even if we're dealing with an XML file, we want to point the browser to the base filename with an
		 * HTML extension since XML files are rendered to HTML by Open Edit.
		 */
	...
	public void replaceAll(WebPageRequest inReq) throws Exception
	{
		...
		if (ffindText != null && path != null)
		{
			...
			// else
			// {
			// throw new OpenEditException( "The 'replace' action only works with file
			// system repositories" );
			// }
	...
	public void loadVue(WebPageRequest inReq) throws Exception
	{
		...
		String editorpath = inReq.findPathValue("localopencommand");
		Runtime.getRuntime().exec(new String[] {editorpath, "--launcher.openFile", absolutepath,
			// "path/to/file2.txt",
			// ...
		});
	}
